...
main( int argc, char **argv )
{
  ...
  c1 = malloc(sizeof(double)*pixnum);
  c2 = malloc(sizeof(double)*pixnum);
  c3 = malloc(sizeof(double)*pixnum);
  ...
  for (long i = 0; i < nthreads; i++)
    {
      int threadcheck = pthread_create(&tid[i], 0, colorcalc, (void*)i);
      if (threadcheck)
	{
  ...
  for (int j = 0; j <nthreads; j++)
    {
      void *retVal;
      int threadcheck = pthread_join(tid[j], &retVal);
      if (threadcheck)
	{
...
void *colorcalc (void * arg)
{
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
	...
	: pixel_dx;
	  ...
	  long iD = (long) arg;
	  /* for every pixel */
	  for(long  px = iD; px<width; px = px + nthreads)
	    {
	      ...
	      for( int py=0; py<height; ++py )
		{
		   ...
		   */
		  int index = (px * height) + py;
		  c1[index] = scaled_color[0];
		  c2[index] = scaled_color[1];
		  c3[index] = scaled_color[2];
