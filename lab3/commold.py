...
class comm:
	...
	def finalprint(self, sup1, sup2, sup3):	
		...
		for i in range(len(self.list1)):
		#	if self.list1[i] == [^ $]* and self.list2[i] and [^ $]* and self.list3[i] == [^ $]*:
		#	if self.list2[i] == [] * len(self.list2) and self.list2[i] == [''] * len(self.list2) and self.list3[i] == [''] * len(self.list3) :
		#	if self.p.match(self.list1[i]) is not None and self.p.match(self.list2[i]) is not None and self.p.match(self.list3[i] is not None):
